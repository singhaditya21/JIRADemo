...
|---|---|---|---|
| 1 | Site is `your-site.atlassian.net`; account has site-admin rights | ✅ VERIFIED 2026-07-20 | `/rest/api/3/mypermissions` → `ADMINISTER`, `CREATE_PROJECT`, `ADMINISTER_PROJECTS` all true |
...
## Roadmap push — Batch B: ITSM chart forms (Part V) — 2026-07-22
...
## Roadmap push — Batch D: NLQ, Sankey, drill & design polish — 2026-07-22

| # | Claim | Status | Evidence |
|---|---|---|---|
| 155 | "Ask the tower" NLQ ships as an honest client-side query parser (roadmap 6.7) | ✅ VERIFIED 2026-07-22 | A static page has no model to run, so instead of faking an LLM, `AskTower` maps a plain-language phrase to a live record predicate over the fields the tower holds — labelled "pattern-based … not an LLM". Verified live: "breached P1 incidents in Database with no KB" parsed to `type=Incident ∧ P1 ∧ SLA breached ∧ KB gap ∧ Database`, showed "2 records in window", and the Drill button opened the drawer to exactly those 2 records. Handles type, priority (P1–P4 or critical/high/…), state (breached/met/open/closed/escalated/reopened/aged >Nd), tower, channel, and root cause, AND-combined. Example chips one-click. |
| 156 | Tier-flow Sankey is live (roadmap D1) | ✅ VERIFIED 2026-07-22 | New `Sankey` inline-SVG primitive (two-column, ribbon width ∝ value) + `TierSankey` panel on OPS Overview/L2: computes tier→tier transition counts from each record's changelog (tof classifier), renders 4 ribbons over 532 total transitions, ribbons drillable to the records that made that transition. Complements the existing TierFlow hop-distribution panel. |
| 157 | Drill polish: column picker + row virtualization (Part III layer c) | ✅ VERIFIED 2026-07-22 | The record list gained a **column picker** (a "columns ▾" menu toggling any column except Key; header + CSV update live — verified via a real ref-click: unchecking "Tier" dropped headers 13→12, drawer stayed open) and **row virtualization** (windowed rendering past 80 rows — a 347-record list renders only ~34 DOM rows, marked "·virtualised", and still reconciles "matches 347 ✓"). Zero console errors. |
| 158 | Design polish: container queries + push-not-overlay drawer (Part VII) | ✅ VERIFIED 2026-07-22 | `.panel` is now a **container-query context** (`container-type: inline-size`), so internals respond to the panel's own width, not the viewport — verified: the same viewport (1306px) renders a span-2 trend-grid (797px panel) at 3 columns while a span-1 scorecard (390px panel) stays at 2. The drill drawer now **pushes instead of overlays** on ≥1000px screens: opening it sets `main` padding-right = the drawer width (577px) and docks the drawer at the right (left 746px, width 560px, transparent — no dimming scrim) so the board reflows beside it and stays usable; below 1000px it reverts to the full overlay + scrim (verified at 375px: overlay left 0, scrim present, no push). |


## Roadmap 100% push — Waves 1–5 — 2026-07-22

Final independent 6-part liveness audit (12 agents, adversarially verified) after the push:
**75.5% live** (equal-item and equal-part), up from ~44%. Per-Part: IV 92%, V 81%, VII 75%,
VI 72%, I+II 72%, III 61%. ~45 new panels/features across Waves 1–5, all verified rendering
with zero console errors and reconciling drills.

| # | Claim | Status | Evidence |
|---|---|---|---|
| 159 | Foundations: real lifecycle timestamps + near-real-time cron + frozen baseline | ✅ VERIFIED 2026-07-22 | Bake emits real seeded `first_response_at`/`escalated_at` + derived `escalation_latency_h`/`l2_dwell_h`/`ola_handoff_h`/`ttr_h` (deployed records confirmed) — unlocks genuine timing metrics without fabrication. CI cron tightened to `*/30` (the roadmap's 15–30 min near-real-time bar, static-host path). Pilot baseline frozen write-once (`{project}-baseline.json`, deployed, frozen_at 2026-07-22). |
| 160 | Part IV OPS catalog 41%→92% | ✅ VERIFIED 2026-07-22 | +13 panels: demand curve, triage-dwell box-plot, escalation-latency histogram (real), time-in-tier, reason×root-cause, SLA-by-priority, KB discipline, resolution-code mix, root-cause pareto, open-by-priority, arrivals-vs-completions, gate-bypass, invariant footer + rule-health board. |
| 161 | Part V ITSM catalog 39%→81% | ✅ VERIFIED 2026-07-22 | +15 panels: incident vol/esc by tower, MTTA-by-priority, OLA handoff (real), SLA outcome donut, SLA type×priority heatmap, request/approval aging, RCA cycle (real), CSAT×SLA, CSAT by tower, queue depth, change success, problem backlog, DQ board, deflection funnel. |
| 162 | Part VI insights 17%→72% + Part III/VII polish | ✅ VERIFIED 2026-07-22 | Part VI: headline banner, pilot-progress baseline→target gauges, at-risk-today burn table, ask-the-tower NLQ, anomaly/recommendations/forecast/league/what-changed/scorecard. Part III: column picker, virtualization, 2-level breadcrumb, JQL preview, status-flow ribbon, +4 cohort dims. Part VII: container queries, push-not-overlay drawer, dense grid. |
| 163 | The honest ceiling is stated, not faked | ✅ VERIFIED 2026-07-22 | Not driven to a fake 100%: real-LLM free-text NLQ (pattern parser ships instead), external Jira-config items (19 named queues / portal-availability badge / dashboard fold-in → closest record-derived equivalents), real CSAT/CES survey (labelled modelled proxy), 18-month outcome gates (need real improvement over time), multi-instance MSP + KB/Problem auto-generation + predictive breach-risk (need a second instance / LLM / live clocks) remain honestly out of reach on a static seeded demo. |


## Roadmap polish wave — Part III + VI buildable items — 2026-07-22

| # | Claim | Status | Evidence |
|---|---|---|---|
| 164 | Drill drawer polish: pin, full-width, keyboard nav, record-walk, per-column filters, 4-level breadcrumb, SLA clock | ✅ VERIFIED 2026-07-22 | Verified via the accessibility tree on an open drill: header carries 🔗 share / 📌 pin / ⤢ full-width tools + Close; record list carries Choose-columns + **Per-column filters** + copy-keys + CSV. Breadcrumb is now a clickable trail (title › filter-crumbs › record) with per-frame pop. Keyboard: Esc back/close, ↑/↓ walk records, ←/Backspace back, f full-width, p pin (window keydown via navRef). RecordDetail gained prev/next record-walk (pos "N of M") + an SLA-clock indicator (running/paused/stopped + pauses-in-path, sequence-honest). Caught+fixed a React #310 (a useState placed after the drawer's early return); app now renders all 45 panels cleanly on the current bundle. |
| 165 | Shareable drill URLs for data-only drills (Part III §6) | ✅ VERIFIED 2026-07-22 | Data-only drill types (metric/kbgap/statusgroup/tower) encode to `#/…/d/<base64>`; parseHash decodes + restores on load; the drawer shows a 🔗 copy-link button for those types (verified present in the tree). Record-filter drills carry a predicate function and are deliberately not link-encoded. |
| 166 | Part VI: materiality-ranked insights + quantified recommendation impact | ✅ VERIFIED 2026-07-22 | buildInsights now sorts by severity then materiality = |gap|×denominator — verified live: the KB-gap insight (79×1.5) ranks #1 above the individual metric gaps. Recommendations carry stated impact ranges — verified "Close the KB gap … — est. +6–11 pts FTR". |
| 167 | Panel layout gaps fixed via masonry row-span packing | ✅ VERIFIED 2026-07-22 | The grid sized rows to the tallest panel, so short span-2 panels left large transparent gaps under taller row-mates (user-reported). Replaced with JS masonry (1px auto-rows, zero row-gap, each panel's grid-row-end = content height +16px in a useLayoutEffect; batched read/write; ResizeObserver + resize/load re-pack; idempotent under align-items:start). Verified LIVE: worst vertical gap 360px→17px, panels content-height (no stretch), stable across resize cycles (no loop), zero console errors on a fresh load. |


## DeliveryIQ P0 — the real SFC Jira build — 2026-07-22

| # | Claim | Status | Evidence |
|---|---|---|---|
| 168 | `jira_config/sfc_build.py` provisions the real SFC project idempotently, following the proven OPS shapes | ✅ VERIFIED 2026-07-22 (static) | Self-contained provisioner: SFC company-managed software project (`ensure_project`, POST /rest/api/3/project); "Salesforce Config Request" (standard) + "Org Deploy" (subtask) issue types + dedicated scheme bound to SFC (`ensure_types`/`ensure_scheme`, matching issuetypes.py); DeliveryIQ custom fields (Delivery Squad, Change Risk, CAB Approval, Deploy State, Config Health, Deploy Source, Target Orgs, Config Component Type, Package Ref, Coord Conflicts/Dependencies, Health Checked At, Build Tested/Compliance Authorized/Evidence/Evidence Pack Ready) with options via the generic `ensure_options`, added to SFC screens, new contexts scoped to SFC best-effort; shared OPS fields (Impact/Urgency/4 dates/L2 Analyst) reused by name and left global (never scoped — scoping would strand them on OPS); 11 statuses + a best-effort workflow with a CAB gate (`system:validate-field-value`/`fieldRequired` on "Request CAB", the OPS-gate analogue) + a printed transition table + UI fallback. Owns `.sfc_state.json` via `merge_state`. Distinct UUID namespace from OPS. Import + py_compile clean; **no token locally → cannot run live, runs via the Action (dry-run first)**. |
| 169 | `fixtures/sfc_seed.py` seeds real requests + Org Deploy sub-tasks matching the preview schema exactly | ✅ VERIFIED 2026-07-22 (static) | REUSES `app.sfc_seed.build()` as the single source of truth, so a real and a preview record are identical by construction. Per request: create the Salesforce Config Request, walk the workflow to the record's status (transition-by-name, degrades honestly if the workflow isn't bound — reports "0 reached target status"), then one Org Deploy sub-task per target org carrying Deploy State/Config Health/Health Checked At/Deploy Source. Field payload gated by per-issue-type `createmeta`; native priority set only if the SFC scheme offers the name. Cross-checks pass: every gate field is a real field, every seeder PATHS status is a transition target, every preview status is covered by PATHS, every mapped field name exists in the build schema. |
| 170 | `sfc-build.yml` is a dry-run-first, injection-safe one-click Action | ✅ VERIFIED 2026-07-22 | workflow_dispatch, `dry_run` defaults true; modes build/seed/all. All inputs passed via `env:` and read as shell vars (never `${{}}`-interpolated into the token-bearing step); `seed_count` validated as a positive integer; the dry gate is a bash `case` on the string (not a GH `${{ == }}` ternary), mirroring the hardened link-problems.yml. Dry "all" skips seed when no `.sfc_state.json` exists yet. YAML validated. Token stays a CI secret. FOLLOW-UP (not P0): point `app/export_pages.py` at the real SFC project and drop the preview-seed step from pages.yml. |


## DeliveryIQ P0 hardening — the workflow actually binds now — 2026-07-22

Driven by a 5-agent research+adversarial-review workflow (2 API researchers on developer.atlassian.com + 2 file reviewers + a verify pass): 9 findings, 1 confirmed-high (the central gap), fixes applied and re-verified by a focused API-shape agent.

| # | Claim | Status | Evidence |
|---|---|---|---|
| 171 | The SFC workflow is now CREATED **and BOUND** to the project — statuses are reachable with no UI step | ✅ VERIFIED 2026-07-22 (API-shape confirmed) | The confirmed-high gap: `sfc_build` created the workflow via POST /rest/api/3/workflows/create but never bound it, so the SFC project kept the template's To Do/In Progress/Done and the seed could never leave the initial status. Fix: added `ensure_workflow_scheme` — POST /rest/api/3/workflowscheme `{name, defaultWorkflow: "SFC DeliveryIQ Workflow"}` (classic scheme binds the workflow by NAME) then PUT /rest/api/3/workflowscheme/project `{workflowSchemeId, projectId}`. That PUT is **synchronous (204) with no async migration because it runs against the still-EMPTY project** — which is exactly why `sfc_build` must precede `sfc_seed`. Idempotent: `current_scheme_id` skips the PUT when already bound; on a re-run after issues exist the PUT is refused and it falls back to the printed transition table. Research + a dedicated verify agent confirmed all four API shapes (create/assign/query/default-scheme-returns-no-id) against developer.atlassian.com. Records workflow_scheme_id in state; `workflow_created` now guards the dry-run None sentinel (matches workflow.py). |
| 172 | Shared-field ids are disambiguated (no JSM "Urgency" look-alike) | ✅ VERIFIED 2026-07-22 | Reused OPS fields (Impact/Urgency/L2 Analyst/4 dates) were resolved by a last-wins name→id map; shared/fields.py documents two custom fields named "Urgency" on this instance, so the seeder could get the JSM look-alike whose option set differs → every issue-create 400s. Fix: `build_fields` now resolves shared fields through `shared.fields.resolve()` (the same type+description-tag disambiguation the app uses at runtime), falling back to create-by-name only if OPS's schema is absent. |
| 173 | Seeder createmeta uses the non-deprecated endpoint + degrades safely; sub-task/priority gaps warn loudly | ✅ VERIFIED 2026-07-22 | `createmeta` now prefers the paginated GET /createmeta/{key}/issuetypes[/{id}] (the classic ?expand form is being removed from Cloud), classic as fallback, and returns None → the seeder ABORTS with a clear message rather than mass-creating field-less shells. `at_target` now RE-READS the issue status (reached==len(path) falsely counted Intake-target records as landed even when the workflow was unbound). If the Org Deploy sub-task create screen is missing the deploy/health fields, or native priority isn't offered, the seeder warns loudly (Model A data / priority views would otherwise blank silently). |
| 174 | Removed the no-op "scope contexts" step and the dangerous all-screens fallback | ✅ VERIFIED 2026-07-22 | `scope_fields_to_sfc` was a guaranteed no-op (Jira rejects assigning a project to a field's global default context) that mislabelled itself as scoping — removed; fields honestly use global contexts (visible only where placed on a screen). `add_fields_to_screens` no longer falls back to ALL screens when no SFC-named screen is found (that could spray SFC fields onto OPS/ITSM screens) — it warns and skips instead. |


## DeliveryIQ P0 EXECUTED — the SFC lens is LIVE on real Jira — 2026-07-22

Ran the whole P0 against the real instance via the sfc-build.yml Action (token stayed a CI secret; triggered + monitored with `gh`). Three real fixes surfaced only by running it, then the lens flipped preview→live.

| # | Claim | Status | Evidence |
|---|---|---|---|
| 175 | The real SFC Jira project is provisioned, seeded, and the lens is LIVE | ✅ VERIFIED LIVE 2026-07-22 | Executed via the Action: **build** created project SFC + issue types (10059/10060) + 16 new fields + 7 shared (disambiguated: Impact=cf_10004, Urgency=cf_10044) + 11 statuses + the workflow + **bound the workflow scheme (10078)** → "Intake/Build/Review/Deploy/Audit now reachable"; **seed** created **64 requests + 181 Org Deploy sub-tasks, 0 failed, all 64 reached their target status** (proves the workflow bind works). Two run-only bugs found and fixed mid-flight: (a) field/screen matching missed SFC's screens because /screens paginates past 100 on an instance with OPS+ITSM+JSM — replaced with screen-scheme resolution (project→issue-type screen scheme→screen schemes→screens; landed 46 associations on screens 10254/10255); (b) attempt_workflow now treats a re-run "already exists" 400 as success. Seeder also made state-file-independent (resolves field ids by name) so mode=seed runs standalone. |
| 176 | export_pages bakes the real SFC project; the preview seed is retired to a fallback | ✅ VERIFIED LIVE 2026-07-22 | New `app/sfc_export.py` reads project=SFC read-only (one paginated /search/jql with changelog), splits requests vs Org Deploy sub-tasks, groups deploys under parents (org recovered from the sub-task summary), derives stage from status, emits SFC-{30,90,180}.json + SFC-records.json with **preview:false** — schema parity with app/sfc_seed verified key-for-key. pages.yml preview-seed step is now a fallback (skips when the live bake wrote SFC-records.json). Pages run 29914973502: "SFC live bake (64 requests)", "live SFC bake present — skipping the preview seed", deploy success. |
| 177 | The deployed lens shows live data with an honest "Live" banner | ✅ VERIFIED LIVE 2026-07-22 | Deployed SFC-90.json: preview=False, volume 29, deploy_success_pct 27.2% GAP (22/81), lead_time_d 31.3d. Records: preview=False, count 64, real per-org deploys (org/state/health/source) + status spread across all 9 statuses. Browser: the SFC tab renders a green **LIVE** banner (replacing the amber Preview one) with the honest "deploy state & config health read Source=Seeded until a real CI/CD writeback runs" caveat; scoreboard (29/81/27%/8/31.3d/7) and five-stage funnel (Intake 3, Build 4, Review 4, Deploy 11, Audit 7) all render real Jira data. |
| 178 | The deploy/health writeback is built and RUN — values maintained on the Jira sub-tasks | ✅ VERIFIED LIVE 2026-07-22 | New `app/sfc_writeback.py` + `.github/workflows/sfc-writeback.yml` (dry-run-first, injection-safe): enumerates the SFC Org Deploy sub-tasks, models each org's deploy state + config health, and WRITES Deploy State / Config Health / Health Checked At (=run time) / Deploy Source back to Jira. Ran via the Action: **90 written, 91 skipped (not deployed), 0 failed**. The Jira writeback + timestamp are REAL, so the health board's staleness signal is genuine. |
| 179 | No live Salesforce → deploy/health honestly labelled MODELLED (not "CI writeback") | ✅ VERIFIED LIVE 2026-07-22 | User: "There is no SF Instance." The earlier "CI writeback / Salesforce drift probe / simulated until SF creds" overclaimed a Salesforce connection that does not (and won't) exist — DeliveryIQ tracks config requests as Jira issues by design. Relabelled per-org deploy state & config health as **MODELLED** everywhere: `sfc_writeback` drops the SF_* creds gate + NotImplementedError, `probe_org`→`deploy_health_model`, writes Deploy Source="Modelled"; `sfc_build` adds the "Modelled" option; preview `sfc_seed`, `sfc_export` note, both lens banners, the health board ("model-refreshed" not "CI-written"), the writeback Action, and DELIVERYIQ-SF-CONFIG.md §0 (the "health is REAL via a real probe" decision → MODELLED). Re-ran build (+1 option) → writeback (restamp) → Pages bake. Deployed SFC-records.json: source split **{Modelled: 90, Seeded: 91}** (no "CI writeback"). Browser: Live banner "...deploy state & config health are **modelled** — no live Salesforce connection..."; Config health "**Modelled** post-deploy health per org — no live Salesforce". Seam kept for the hypothetical future real org (swap `deploy_health_model`, write "CI writeback"). |


## Closeout — 4 buildable pointers (real) + out-of-reach as labelled mocks — 2026-07-22

Confirmed approach with the user first (inline MOCK badges; backend deploy-ready + steps). All verified LIVE on the deployed site (Pages run 29929655032 + browser DOM checks).

| # | Claim | Status | Evidence |
|---|---|---|---|
| 180 | SFC reaches parity: snapshot history + frozen baseline + trend/bottleneck/outcome panels (all REAL) | ✅ VERIFIED LIVE 2026-07-22 | `sfc_export` now writes **SFC-history.json** (daily point: deploy_success/healthy/lead) + write-once **SFC-baseline.json**; `pages.yml` commits `*-baseline.json` too so SFC's is truly frozen (bake: "SFC-history.json (1 point)", "SFC-baseline.json (pilot baseline frozen)"; committed back as 4fdc12b). New REAL panels verified in the DOM on the deployed SFC lens: **Delivery trend** (Sparklines from history), **Deploy failures by org** (per-org fail/rollback from real org_deploys, drillable), **Lead time & where work waits** (real lead-time histogram + oldest-open-by-stage bottleneck), **Outcomes vs baseline** (scoreboard vs frozen baseline). 14 panels on the SFC lens. |
| 181 | Out-of-reach features shipped as clearly-labelled MOCKS (inline, bright MOCK badge) | ✅ VERIFIED LIVE 2026-07-22 | 5 mock panels, each with a `.mock-badge` + dashed-magenta `.panel.mock` border + an explicit "illustrative / not real" caption: **MockNlqAnswer** (LLM answer templated from real numbers), **MockCsatFeed** (survey verbatims over real resolved tickets + modelled CSAT), **MockBreachRisk** (deterministic score over REAL open tickets, drillable to the real row), **MockMultiInstance** (MSP tenant switcher — this tenant real, others mock), **MockOutcomes18mo** (18-mo projection from the real current rate). DOM verified: OPS Overview shows 5 mock headings all suffixed "MOCK", 5 badges, 5 `.panel.mock`; SFC lens shows 3 SFC-compatible mocks. Browser screenshot confirms the MOCK badge + honest "there is only one instance here" caption + real-vs-mock tenant tabs render. Nothing dressed as real — matches the tower's modelled-labelling discipline. |
| 182 | SFC writeback scheduled; backend serves SFC; deploy steps documented | ✅ VERIFIED 2026-07-22 | `sfc-writeback.yml` runs every 6h and APPLIES on schedule (dry-first on manual) via a GITHUB_EVENT_NAME check — keeps the modelled-health staleness clock fresh. `app/server.py` PROJECTS now includes SFC and serves `/api/tower?project=SFC` + `/api/records?project=SFC` via `fetch_sfc_records`/`sfc_model` (deploy itself is the user's action — needs their host + token; DEPLOY-BACKEND.md updated with the SFC endpoint). webapp `vite build` passes. |
| 183 | Agent Activity & Orchestration board — real workload + handoffs, modelled outputs, mock live feed | ✅ VERIFIED LIVE 2026-07-22 | The flat agent ledger undersold DeliveryIQ's three-agents pitch and slightly overclaimed (implied agents run). Rebuilt as a 3-tier honest board on the SFC lens. **AgentWorkload (REAL):** each of Build/Compliance/Coordination's live in-flight count from the real current stage of open requests (deployed: Build 7 / Compliance 6 / Coordination 10), drillable per agent + stage. **AgentHandoff (REAL):** pipeline as agent-owned stages (Intake/Build→Build, Review/Audit→Compliance, Deploy→Coordination) with live WIP + a bottleneck flag (Deploy, 10 open, "⌛ waits here"), and real agent→agent handoff counts from the Jira changelog (deployed: Build→Compliance 22 · Compliance→Coordination 18 · Coordination→Compliance 7). **AgentLedger:** kept but relabelled **MODELLED** — the outputs (test-pass/evidence/conflicts) are modelled, no agents run. **MockAgentFeed (MOCK badge):** deterministic illustration of an autonomous-agent event stream over real requests (12 events). Honesty tiers verified in the deployed DOM + screenshot: MODELLED badge on the ledger, MOCK badge on the feed, real counts on workload/handoff. |

## Audit remediation — honesty, integrity, tests — 2026-07-23

Driven by the 5-agent gap audit (65 findings, 16 confirmed-serious) + a spec-extraction and security workflow.

| # | Claim | Status | Evidence |
|---|---|---|---|
| 184 | The three honesty violations are closed | ✅ VERIFIED LIVE 2026-07-23 | (a) **Evidence-pack theatre** — spec §4.4 calls a stored "Ready" flag "the precise dishonesty this lens is meant to expose"; the bake was reading it straight through. Now COMPUTED (`_evidence`) as a conjunction over real evidence, ranging over TARGET ORGS (not just existing sub-tasks), carrying `evidence_pack_claimed` vs `evidence_pack_ready` + `evidence_missing[]`. LIVE: 13 claim ready, 12 are, **7 overclaimed** (e.g. SFC-5 "not deployed in Dev"). (b) **Staleness guard made real** — four docstrings + the banner claimed a stale verdict reads Unknown; nothing checked the clock. `_apply_staleness` forces >24h/never-checked to Unknown. (c) **Volume mismatch** — masthead said 64 while panels showed 8/29; one record set is now windowed per model (verified 8/29/64 both sides). |
| 185 | SFC scoreboard measures the right thing, on the OPS contract | ✅ VERIFIED LIVE 2026-07-23 | deploy_success counted every org cell INCLUDING "Not started" (an unshipped request dragged the rate down — 27% live). Now over ATTEMPTED deploys (55% live), plus the spec's PROD-specific tile, and rollback/redeploy as anti-gaming counter-metrics. `Redeployed` (spec §1, never built as a field) is DERIVED from the real changelog. SFCKpi is a real KPI strip: num/den + target + PASS/GAP + drill per tile. One scoreboard implementation shared by live bake and preview. |
| 186 | SFC inherits the tower's integrity discipline | ✅ VERIFIED LIVE 2026-07-23 | `model.invariants` (7 checks) now emitted and rendered by the existing InvariantFooter on the SFC lens. LIVE on real data: 4 PASS (stage buckets partition 29/29; health slices sum 81/81; every cell carries a Source 81/81; staleness guard held) + 3 WART-CATCHERS flagged not hidden (4 Audit-stage with an undeployed org, Cancelled guarded out; 4 deploys with no recorded CAB approval; 4 evidence overclaims). Drill drawer made lens-aware (SFC cohort dims + columns, multi-valued fields counted per element). Deploy Failed no longer vanishes from WIP (`is_open = not is_done`). |
| 187 | Test suite exists — the audit's BLOCKER closed | ✅ VERIFIED 2026-07-23 | `tests/test_sfc.py` + `pytest.ini`: **40 tests, all passing**, no Jira/token/network. Pins every honesty guarantee and the exact regressions found: evidence conjunct-by-conjunct + the target-org hole, staleness (fresh/stale/never-checked/no-mutation), the Not-started denominator bug, PROD-only success, verdict direction, redeploy derivation, health-model determinism, stage-map coverage, preview volume==client-windowing, Deploy-Failed-stays-open, preview↔live schema parity, and that the preview actually exercises the staleness guard (it had hardcoded `stale=False`, making that invariant a free pass — 47 stale cells now surface). |
| 188 | Security review run; six fixes applied | ✅ VERIFIED 2026-07-23 | Injection vector confirmed CLEAN (all inputs via `env:`, no `${{ }}` in any `run:` body). Fixed: raw Jira error text (tenant URL + response body) was written into `index.json` which ships to a PUBLIC Pages site → class name only; the SFC bake bypassed the repo's own PII control (named reviewer verbatim) → honours PSEUDONYMISE_ANALYSTS, now ON in pages.yml (renders "Reviewer 01"); writeback `notifyUsers=false` (was notifying watchers 4x/day) + refuses to overwrite cells a human/real integration owns; `app/server.py` optional API_TOKEN bearer gate + generic 500s; quoted git ref; seed_count bounded 1..500. REMAINING (repo settings, user's call): approval environment for the write path, split tokens by privilege. |

| # | Claim | Status | Evidence |
|---|---|---|---|
| 189 | The lens's own rules are now tested, and both suites run in CI | ✅ VERIFIED 2026-07-23 | The at-risk predicates, stall thresholds, agent map and stage-of-flight flow decide what the lens ASSERTS, but lived inline in panels.jsx — untestable without a DOM, so the bake had 45 tests while the rules the user reads had none. Extracted to `webapp/src/sfc-logic.js` (pure: no React, no DOM); panels.jsx imports and renders. `sfc-logic.test.js`: **33 bun tests** pinning the things that rot silently — per-stage (not global) thresholds, inclusive at the boundary; case-exact deploy-state matching ("Rolled back" ≠ "Rolled Back"); rolled_back does NOT fire on a merely-failed deploy (deploy_rollup collapses them, which would break the counter-metric pairing); case-insensitive risk; Deploy-Failed IS in the queue but a done request is not; distinct union ≤ summed hits; same-node hops carry no flow; review ping-pong + post-audit rollback register as backward leakage; unknown statuses ignored rather than crashing a panel; empty input never throws. New `.github/workflows/ci.yml` runs pytest AND bun test AND the bundle build on every push/PR (no secrets, contents:read). **78 tests total, CI green on both jobs.** |
| 190 | Light-theme and mobile verified — two real defects found and fixed | ✅ VERIFIED LIVE 2026-07-23 | Measured, not eyeballed. **MOBILE:** the page scrolled sideways — the header `.controls` bar is 480px and never wrapped, overflowing a 375px viewport by **142px**, violating the tower's own "body must never scroll horizontally" rule. Now wraps + width-capped → **overflow 0**. No panel was at fault; every SFC/agent/mock panel already fit, and the one wide table correctly scrolls inside its own `overflow-x:auto` container. **LIGHT THEME:** `--muted` (#64787e) drives `.why`/`.int-note` — the caption under every panel heading — and measured **4.42:1**, just under the 4.5 WCAG AA bar. Darkened to #5c6f75 → **5.03:1**; dark theme untouched. Everything added in this workstream already passed AA in light mode as measured: mock-badge 5.86, risk-chip 4.99, reason-chip 8.11, modelled-badge 8.11, pipe-agent/agent-name 6.06, feed-agent 5.23. |
