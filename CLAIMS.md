...
| 178 | The CI/CD writeback + drift probe is built and RUN — deploy state & config health are now pipeline-owned in Jira | ✅ VERIFIED LIVE 2026-07-22 | New `app/sfc_writeback.py` + `.github/workflows/sfc-writeback.yml` (dry-run-first, injection-safe): enumerates the SFC Org Deploy sub-tasks, probes each org, and WRITES Deploy State / Config Health / Health Checked At (=run time) / Deploy Source="CI writeback" back to Jira. Ran via the Action: dry-run clean (SIMULATED probe, 181 sub-tasks), then real — **90 written (Source=CI writeback), 91 skipped (not deployed), 0 failed**. Pages re-bake + deployed SFC-records.json: source split **{CI writeback: 90, Seeded: 91}**, all 90 CI cells stamped 2026-07-22. Browser health board: **CI-written 40/81 · stale 0** (fresh timestamps), health donut 19% healthy (Healthy 15/Degraded 5/Failing 10/Unknown 51). The Salesforce probe is a documented SEAM (`probe_org`): deterministic simulation until SF_INSTANCE_URL/SF_CLIENT_ID/SF_CLIENT_SECRET secrets are set — the writeback is real (real Jira mutations + timestamps), the health VALUES are modelled, and the run log + lens note say so. Banner/export note updated: "Source = CI writeback … the Salesforce drift probe is simulated until SF credentials are set". |
