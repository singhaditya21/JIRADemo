...
| 31 | An existing Jira Software project cannot be converted to a service project | ⚠️ UNVERIFIED | Project type is understood to be fixed at creation, so adopting JSM means rebuilding `OPS` as a service project. Cheap because the build is scripted, but confirm before planning around it. |
