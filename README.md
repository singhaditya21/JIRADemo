...
python3 -m app.export_pages       # bake the model to webapp/public/data/*.json (for GitHub Pages)
...
- **UI-only, each confirmed by probe rather than assumed:** workflow validators (so the escalation gate must be clicked, not scripted); SLA metrics and goals (a 56-path sweep found zero endpoints); agent queue **deletion** (no REST or GraphQL route — every queue created is permanent); request type update, grouping and form fields; portal branding and knowledge base. Two automation *component types* stayed UI-only — the `field-changed` trigger (won't enable over the API) and the send-email recipient list — so two rules use faithful enable-able equivalents: priority derivation runs on the generic *issue-updated* trigger, and the major-incident rule *assigns* the P1 to the Major Incident Manager (firing Jira's own notification) rather than emailing (CLAIMS #106-108).
