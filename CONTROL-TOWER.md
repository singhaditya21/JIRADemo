...
| **Automation rules** | 🟡 **Internal API — built & enabled** | The *public* `/rest/api/3/automation/rules` → 404, but the Automation **internal** API (`.../pro/rest/{projectId}/rule`) takes create + enable. All seven rules are live and ENABLED via `automation/build_rules.py`; component value schemas were discovered by round-trip (`automation/schema/component-schemas.md`). The `field-changed` trigger and the send-email recipient list stayed UI-only (won't enable over the API), so two rules use faithful equivalents: priority derivation runs on the *issue-updated* trigger, and the major-incident rule *assigns* the P1 to the MIM (firing Jira's own notification) instead of emailing. |
